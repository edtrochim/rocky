...
- rel: allows
  id: enum:RootingStatusEnum
  path: ../enums/RootingStatusEnum.md
- rel: allows
...
Herbaceous Growth Form Element of Forbs Subtype

LChS reference name `LC_Forbs` (schema type `LC_ForbsType`). LCCS3 `xsi:type`: `LC_Forbs`.
...
| `forbsStatus` | RootingStatusEnum | `nonRooted`, `Rooted` |  | Forbs Status |
| `forbsLeafType` | xs:string |  |  | Forbs Leaf Type |
